...
public:
	...
	Fraction copy(const Fraction& f)
	{
		Fraction f2;
	...
	Fraction operator+(const Fraction& f)
	{
		...
		Fraction f_new = copy(f);
		commonDenominator(f_this, f_new);
		Fraction f2;
		f2.denominator = f_this.denominator;
		f2.numerator = f_this.numerator + f_new.numerator;
		reduction(f2);
		return f2;
	}
	Fraction operator-(const Fraction& f)
